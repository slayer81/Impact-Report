...
def inefficient_apps(url, auth_token, count: int):
    ...
    # apps_url = url + '/api/v1/apps/events/inefficient_apps'
    ...
    # print("\tOld method returned results of: {}".format(search_response))
    print("\tOld method returned a dataset containing: {}".format(search_response.keys()))
    if 'total' in search_response.keys():
        print("\tOld method returned key \"total\", with value of: {}".format(search_response['total']))
